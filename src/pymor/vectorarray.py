...
try:
    from pymor.dealii import HAVE_DEALII
...
if HAVE_DEALII:
    from pymor.vectorarrays.dealii import DealIIVectorSpace
...
dealii_vector_array_factory_arguments = \
    zip([102],      # len
        [1],      # dim
        random_integers(1, 123))   # seed
...
dealii_vector_array_factory_arguments_pairs_with_same_dim = \
    zip([1, 43, 102],         # len1
        [37,  9, 104],         # len2
        [34, 32,  13],         # dim
        random_integers(2, 1234) + [42],  # seed1
        random_integers(2, 1235) + [42])  # seed2
